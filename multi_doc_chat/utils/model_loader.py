...
class ApiKeyManager:
    # No key is hard-required at construction — embeddings can run locally (fastembed)
    # and the LLM provider is config-driven. Keys are validated lazily via get() when used.
    REQUIRED_KEYS = []
    OPTIONAL_KEYS = ["GOOGLE_API_KEY", "GROQ_API_KEY"]
    ...
    def __init__(self):
        ...
        # Fall back to individual env vars for any key not supplied via the JSON secret.
        # This runs in all environments (e.g. local dev where only .env is set).
...
class ModelLoader:
    ...
    def load_embeddings(self):
        ...
        Provider "fastembed"/"local" runs on-device (onnx, no API); "google" uses Gemini.
        """
        try:
