...
class ChatIngestor:
    ...
    def build_retriever(self, uploaded_files: Iterable, *, chunk_size: int = 1000, chunk_overlap: int =200, k: int =5, search_type:str = "mmr", fetch_k:int =20, lambda_mult:float=0.5):
        try:
            ...
            from multi_doc_chat.src.document_chat.hybrid_retrieval import persist_chunks
            persist_chunks(chunks, self.faiss_dir)

            # configure search params based on search type
            search_kwargs = {"k":k} 
...
class FaissManager:
    def __init__(self, index_dir: Path, model_loader: Optional[ModelLoader]= None):
        ...
        self._meta: Dict[str, Any] = {"rows": {}} # dict of rows
